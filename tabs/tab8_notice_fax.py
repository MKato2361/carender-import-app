...
def get_event_start_datetime(event: Dict[str, Any]) -> Optional[datetime]:
    ...
    if "dateTime" in start:
        try:
            ...
            if dt.tzinfo is None: dt = dt.tz_localize(timezone.utc)
            return dt.astimezone(JST).to_pydatetime()
        except Exception: return None
    if "date" in start:
        try: return datetime.combine(date.fromisoformat(start["date"]), time.min, tzinfo=JST)
        except Exception: return None
    return None
...
def load_property_master_view(sheets_service: Any, spreadsheet_id: str) -> pd.DataFrame:
    if not sheets_service or not spreadsheet_id: return pd.DataFrame()
    try:
        ...
        if master_df.empty:
            ...
            return merged
        return master_df.merge(basic_df[["管理番号", "物件名", "住所", "窓口会社"]], on="管理番号", how="left")
    except Exception as e:
...
def render_tab8_notice_fax(manager, current_user_email: str) -> None:
            ...
            try:
                ...
                if not events:
                    ...
                    for ev in events:
                        ...
                        if not mgmt: continue
                        
                        # フィルタリング
                        if use_master_filter and pm_idx is not None:
                            if mgmt not in pm_idx.index: continue
                            if str(pm_idx.loc[mgmt].get("貼り紙テンプレ種別", "")).strip() != "自社": continue
    ...
    if cand_df is not None and not cand_df.empty:
        ...
        if target_count > 0:
            if st.button(f"🖨️ {target_count} 件の貼り紙を生成してダウンロード", type="primary", use_container_width=True):
...
def _generate_single_docx(ev, row, pm_idx):
    ...
    if "ASSETNUM" not in tags and mgmt: tags["ASSETNUM"] = mgmt
    ...
    name_for_doc = str(pm_idx.loc[mgmt].get("物件名") or summary or mgmt).strip() if pm_idx is not None and mgmt in pm_idx.index else str(summary or mgmt).strip()
