...
# ✅ Robust import: allow either root-level session_utils.py or utils/session_utils.py
try:
    from session_utils import get_user_setting, set_user_setting
except Exception:  # pragma: no cover
    from utils.session_utils import get_user_setting, set_user_setting  # type: ignore
...
def _save_description_settings(user_id: str):
    ...
    if key in st.session_state:
        set_user_setting(user_id, "description_columns_selected", st.session_state[key])
...
def render_tab2_register(user_id: str, editable_calendar_options: dict, service):
    ...
    saved_calendar_name = get_user_setting(user_id, "selected_calendar_name_outside" if outside_mode else "selected_calendar_name")
    default_index = calendar_options.index(saved_calendar_name) if saved_calendar_name in calendar_options else 0

    select_key = "reg_calendar_select_outside" if outside_mode else "reg_calendar_select"
    if select_key not in st.session_state:
        st.session_state[select_key] = calendar_options[default_index]

    st.selectbox(
        "登録先カレンダーを選択" + ("（作業外予定）" if outside_mode else "（作業指示書）"),
        calendar_options,
        index=calendar_options.index(st.session_state[select_key]) if st.session_state[select_key] in calendar_options else default_index,
        key=select_key,
    ...
    for i, row in df.iterrows():
        ...
        if outside_mode:
            ...
            row_s_key, row_e_key = _normalize_row_times_to_key(
                {"Start Date": start_date_str, "End Date": end_date_str, "Start Time": start_time_str, "End Time": end_time_str},
                all_day_flag,
            )
        ...
        try:
            if existing_event:
                if is_event_changed(existing_event, event_data):
                    update_event_if_needed(service, calendar_id, existing_event["id"], event_data)
                    updated_count += 1
                else:
                    skipped_count += 1
            else:
    ...
    st.success(f"✅ 登録: {added_count} 件 / 🔧 更新: {updated_count} 件 / ↪ スキップ: {skipped_count} 件 処理完了！")
