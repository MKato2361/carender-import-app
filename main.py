...
with tabs[0]:
    ...
    # 説明文を改行を反映させるように修正
    st.info("""
    ☐作業指示書一覧をアップロードすると管理番号+物件名をイベント名としてカレンダーに登録します。登録先カレンダーはドロップダウンリストから選択できます。
    
    ☐イベントの説明欄に含めたい情報はドロップダウンリストから選択（複数選択可）してください。
    
    ☐イベントに住所を追加したい場合は、物件一覧のファイルを一緒にアップロードしてください。
    
    ☐作業外予定の一覧をアップロードすると、イベント名を選択することができます。【備考】を選ぶとわかりやすいと思います。

    ☐作業指示書一覧と作業外予定は同時に登録できないので、別々で登録してください。
    """)
