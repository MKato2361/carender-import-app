    ...
    try:
        ...
        if doc.exists:
            ...
            try:
                ...
                _clear_and_reauth(f"保存トークンの読み込みに失敗しました。再認証します。({e})")
                return authenticate_google()
            ...
            # ✅ トークンを実際に検証（invalid_grant を早期検出）
            try:
                if not creds.valid:
                    creds.refresh(Request())
                    doc_ref.set(json.loads(creds.to_json()))
            except Exception as e:
                _clear_and_reauth(f"保存されたGoogleトークンが無効です。再認証します。({e})")
                return authenticate_google()
        ...
        _clear_and_reauth(f"Firestoreからのトークン取得に失敗しました。再認証します。({e})")
        return authenticate_google()
